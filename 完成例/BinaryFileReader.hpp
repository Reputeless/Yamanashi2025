...
namespace mini
{
	...
	public:
		...
		/// @return 読み込んだバイト数
		std::int64_t read(void* data, size_t size);

		/// @brief ファイルからデータを読み込みます。
		/// @tparam T 読み込むデータの型
		/// @param data 読み込んだデータを格納する変数
		/// @return 読み込んだバイト数
		template <class T> requires std::is_trivially_copyable_v<T> // 関数テンプレートに対する制約（T は trivially copyable でなければならない）
		std::int64_t read(T& data)
		{
			// & 演算子のオーバーロード対策で std::addressof を使用
